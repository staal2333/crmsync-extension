...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'TRIGGER_HUBSPOT_SYNC') {
    console.log('🔵 Triggering HubSpot sync from popup...');
    
    // Run sync in background
    syncFromHubSpot(message.token).then(() => {
  ...
  if (message.action === 'getContactLimit') {
    chrome.storage.local.get(['contacts', 'user'], (result) => {
      ...
      const isNearLimit = limit !== -1 && count >= (limit * 0.9);
  ...
  if (message.action === 'refreshExclusions') {
    fetchUserExclusions(message.token).then(() => {
      sendResponse({ success: true });
    }).catch(() => {
