...
async function generateAndDownloadCsv(contacts) {
  ...
  const filename = `crm-export-${yyyy}-${mm}-${dd}.csv`;
    ...
    // Fallback: return CSV content so caller can handle it if needed
    return { success: true, csvContent: csvContent, filename };
  } finally {
    // Revoke object URL when possible; Chrome may still hold until download finishes.
